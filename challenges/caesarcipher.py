...
class Cipher:
    ...
    def printDecodedHash(self):
        self.decoded = self.decodeHashTable()
        print(self.decoded)
    ...
    def detectEnglish(self, string):
        ...
        '''
        if detect(string) == 'en' and Text(string).language.code == 'en':
            return True
        return False
    ...
    def encodeHashTable(self):
        ...
        return('Encoded sentence: {}'.format(self.encoded))
    ...
    def decodeHashTable(self):
        ...
        return('Decoded sentence: {}'.format(self.decoded))
...
class handleArgs:
    ...
    def getArgs(self):
        ...
        detect_command.add_argument('d_string', action='store', help='String to be decoded')
        ...
        if self.handleFile:
            if os.path.isfile(args['thefile']):
                theFile = open(os.path.join(__location__,args['thefile']), 'r')
...
if __name__ == '__main__':
    ...
    if args.handleDetect:
        guess, string1, string2 = message.detectByFrequency()
        print('Guessed shift of: {}'.format(guess))
        print('Decoded: {}'.format(string2))
    else:
        if args.encodeFlag:
